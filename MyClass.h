...
// Wed Aug 12 17:38:35 2020 by ROOT version 6.13/01
// from TTree cbmsim_reduced/cbmsim_reduced
// found on file: /mnt/pool/nica/6/parfenovpeter/mpd_data/PicoDst/11gev/mpddst_87374997_54_reduced.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Float_t         eta_mc[2345];   //[n_tracks_mc]
   Float_t         pt_mc[2345];   //[n_tracks_mc]
   Int_t           mother_ID_mc[2345];   //[n_tracks_mc]
   Int_t           PDG_code_mc[2345];   //[n_tracks_mc]
   Float_t         px_mc[2345];   //[n_tracks_mc]
   Float_t         py_mc[2345];   //[n_tracks_mc]
   Float_t         pz_mc[2345];   //[n_tracks_mc]
   Float_t         start_x_mc[2345];   //[n_tracks_mc]
   Float_t         start_y_mc[2345];   //[n_tracks_mc]
   Float_t         start_z_mc[2345];   //[n_tracks_mc]
   Float_t         mass_mc[2345];   //[n_tracks_mc]
   Float_t         energy_mc[2345];   //[n_tracks_mc]
   Long64_t        n_tracks_mpd;
   Long64_t        k_tracks_mpd;
   Float_t         eta_mpd[1576];   //[n_tracks_mpd]
   Float_t         phi_mpd[1576];   //[n_tracks_mpd]
   Float_t         theta_mpd[1576];   //[n_tracks_mpd]
   Int_t           TOF_flag_mpd[1576];   //[n_tracks_mpd]
   Float_t         ZDC_energy_mpd[90];
   Float_t         pid_tpc_prob_electron_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_pion_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_kaon_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_proton_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_electron_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_pion_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_kaon_mpd[1576];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_proton_mpd[1576];   //[n_tracks_mpd]
   Bool_t          isPIDTofTpc[1576];   //[n_tracks_mpd]
   Bool_t          isPIDTpc[1576];   //[n_tracks_mpd]
   Float_t         tof_beta_mpd[1576];   //[n_tracks_mpd]
   Float_t         tof_mass2_mpd[1576];   //[n_tracks_mpd]
   Float_t         dEdx_tpc_mpd[1576];   //[n_tracks_mpd]
   Float_t         chi2_mpd[1576];   //[n_tracks_mpd]
   Float_t         chi2_vertex[1576];   //[n_tracks_mpd]
   Float_t         pt_error_mpd[1576];   //[n_tracks_mpd]
   Float_t         theta_error_mpd[1576];   //[n_tracks_mpd]
   Float_t         phi_error_mpd[1576];   //[n_tracks_mpd]
   Float_t         DCA_x_mpd[1576];   //[n_tracks_mpd]
   Float_t         DCA_y_mpd[1576];   //[n_tracks_mpd]
   Float_t         DCA_z_mpd[1576];   //[n_tracks_mpd]
   Float_t         DCA_x_mc[1576];   //[n_tracks_mpd]
   Float_t         DCA_y_mc[1576];   //[n_tracks_mpd]
   Float_t         DCA_z_mc[1576];   //[n_tracks_mpd]
   Int_t           n_hits_mpd[1576];   //[n_tracks_mpd]
   Int_t           n_hits_poss_mpd[1576];   //[n_tracks_mpd]
   Float_t         signed_pt_mpd[1576];   //[n_tracks_mpd]
   Int_t           centrality_tpc_mpd;
   Long64_t        id_from_mc_mpd[1576];   //[n_tracks_mpd]
   Float_t         p_mpd[1576];   //[n_tracks_mpd]
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/mnt/pool/nica/6/parfenovpeter/mpd_data/PicoDst/11gev/mpddst_87374997_54_reduced.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/mnt/pool/nica/6/parfenovpeter/mpd_data/PicoDst/11gev/mpddst_87374997_54_reduced.root");
      }
