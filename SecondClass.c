...
//TFile* FiRec =new TFile (RecenteringFileName.Data(),"read");
 ...
 //   fp=fopen("my_file0.txt","w");
      ...
      TCanvas* c100  =new TCanvas("c100","Profile");
      TProfile* hprof =new TProfile("hprof","Profile PhinHalf",100,0,100);
...
//    }
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	ResN[j-1]=ResN[j-1]+1; 
	hprof->Fill(j*10-5,cos(atan2(Qy901-RecQy90[j],Qx901-RecQx90[j])-atan2(Qy441-RecQy44[j],Qx441-RecQx44[j])));
...
//      }
      ...
      TCanvas* c17 =new TCanvas("c17","graph1");
      Double_t x[8];
      Double_t y[8];
      for(int j=0;j<8;j++)
      {
      x[j]=(2*j+1)*5;
      y[j]=sqrt(ResPhin[j]/ResN[j]);
      }
      TGraph* gr =new TGraph(8,x,y);
      gr->SetMarkerSize(1.5);
      gr->SetMarkerStyle(21);
      gr->SetTitle("Resolution for half detector vs centrality");
      gr->GetXaxis()->SetTitle("centrality,%");
      gr->GetYaxis()->SetTitle("Resolution");
      gr->Draw("AP");
      c17->Print("ResPhinHalf.pdf");

      TCanvas* c18 =new TCanvas("c18","graph2");
      Double_t xend[8];
      Double_t yend[8];
      for(int j=0;j<8;j++)
      {
      ...
      {
      xsredn=(xmax+xmin)/2;
      ys=(sqrt(atan(1)*4)/(2*sqrt(2)))*xsredn*exp(-xsredn*xsredn/4)*(ROOT::Math::cyl_bessel_i(0,xsredn*xsredn/4)+ROOT::Math::cyl_bessel_i(1,xsredn*xsredn/4));
      if(ys<=sqrt(ResPhin[j]/ResN[j]))
      {
      xmin=xsredn;
      }
      if(ys>=sqrt(ResPhin[j]/ResN[j]))
      ...
      while(abs(sqrt(ResPhin[j]/ResN[j])-ys)>0.0001);
      xend[j]=(2*j+1)*5;
      yend[j]=(sqrt(atan(1)*4)/(2*sqrt(2)))*sqrt(2)*xsredn*exp(-xsredn*xsredn*2/4)*(ROOT::Math::cyl_bessel_i(0,xsredn*xsredn*2/4)+ROOT::Math::cyl_bessel_i(1,xsredn*xsredn*2/4));
      }
      TGraph* gr2 =new TGraph(8,xend,yend);
      gr2->SetMarkerSize(1.5);
      gr2->SetMarkerStyle(21);
      gr2->SetTitle("Resolution for all detectorn vs centrality");
      gr2->GetXaxis()->SetTitle("centrality,%");
      gr2->GetYaxis()->SetTitle("Resolution");
      gr2->Draw("AP");
      c18->Print("ResPhinAll.pdf"); */

      hprof->Draw();
      c100->Print("ProfileResPhinHalf.pdf");
      c100->Print("ProfileResPhinHalf.root");

 Double_t ResPhinHalf1[8]= {0.0894,0.1378,0.4025,0.5138,0.6663,0.7266,0.7629,0.7759};
Double_t ResPhinHalfErr[8] = {0.0894,0.058,0.0199,0.0146,0.009,0.0069,0.0066,0.0064};
Double_t xend[8];
Double_t yend[8];
Double_t yerr[8];
...
for(int j=0;j<8;j++)
      ...
      while(abs(ResPhinHalf1[j]-ys)>0.0001);
      xend[j]=(2*j+1)*5;
      yend[j]=(sqrt(atan(1)*4)/(2*sqrt(2)))*sqrt(2)*xsredn*exp(-xsredn*xsredn*2/4)*(ROOT::Math::cyl_bessel_i(0,xsredn*xsredn*2/4)+ROOT::Math::cyl_bessel_i(1,xsredn*xsredn*2/4));
      yerr[j]=yend[j]*ResPhinHalfErr[j]/ResPhinHalf1[j];
      }

TCanvas* c200 =new TCanvas("c200","graph2");
 TGraph* gr2 =new TGraphErrors(8,xend,yend,xerr,yerr);
      gr2->SetMarkerSize(1.5);
      gr2->SetMarkerStyle(21);
      gr2->SetTitle("Resolution for all detectorn vs centrality");
      gr2->GetXaxis()->SetTitle("centrality,%");
